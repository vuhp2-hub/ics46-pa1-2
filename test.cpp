...
// submit it).
//
// Builds a MenuModel from a .menu file (and optional .chosen file), then
// exercises the PA 1,2 functions and prints a readable report. The Gradescope
// autograder does NOT diff this output -- it calls your functions directly and
// checks their RETURN VALUES against the reference. Use this program to
// sanity-check your work locally:
//
...
int main(int argc, char **argv) {
    ...
    // // A sort/search order (reverse course order: last course most
    // significant). Course *order = new Course[n]; for (int k = 0; k < n; ++k)
    // {
    //     order[k].courseIdx = n - 1 - k;
    //     order[k].numDishes = model.dishes(n - 1 - k);
    // }

    // if (total > 0) {
    //     Meal target = meals[total - 1]; // a meal we know is present
    //     // LINEAR search scans the UNSORTED array -- O(N); it only needs
    //     // Meal::operator== (no sort order).
    //     std::cout << "linear_search_index (unsorted): "
    //               << linearSearchForMeal(meals, static_cast<int>(total),
    //               target)
    //               << "\n";

    //     // MERGE SORT (Unit 2) -- sort a COPY of the still-unsorted meals in
    //     O(N
    //     // log N); the fast counterpart to the O(N^2) bubbleSortMeals below.
    //     It
    ...
    //               << binarySearchForMeal(meals, static_cast<int>(total),
    //               target,
    //                                      order, n)
    //               << "\n";
    // }
    ...
    // // The product of the first two pairing tables (if there are at least
    // two). if (model.numTables() >= 2) {
    //     PairingFactor prod = model.factorAt(0) * model.factorAt(1);
    //     std::cout << "product(f0,f1) scope:";
    //     for (int j = 0; j < prod.scopeSize(); ++j)
    //         std::cout << " " << prod.courseAt(j);
    //     std::cout << "\nproduct(f0,f1) scores:" << std::setprecision(17);
    //     for (long long e = 0; e < prod.numEntries(); ++e)
    //         std::cout << " " << prod.scoreAt(e);
    //     std::cout << "\n";
    // }
    // return 0;
}
